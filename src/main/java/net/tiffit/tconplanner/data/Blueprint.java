...
public class Blueprint {
    ...
    public int getFreeSlots(SlotType slotType) {
        ...
        int creative = creativeSlots.getOrDefault(slotType, 0);
    ...
    public Blueprint(TCTool tool) {
        ...
        parts = ToolPartsHook.parts(toolDefinition).stream()
                .filter(Objects::nonNull)
                .toArray(IToolPart[]::new);
        ...
        if(applyMods) {
            for (ModifierInfo info : modStack.getStack()) {
                ...
                if (info.count != null) {
                    ...
                    SlotType slotType = info.count.type();

                    int count = info.count.count();
                    stack.getPersistentData().addSlots(slotType, -count);
                }
            }
    ...
    public RecipeResult<ToolStack> validate(RegistryAccess access) {
        ToolStack ts = ToolStack.from(createOutput(false));
        RecipeResult<ToolStack> result = null;
        for (ModifierInfo info : modStack.getStack()) {
            IDisplayModifierRecipe recipe = info.recipe;


            RecipeResult<LazyToolStack> rs = ((ITinkerStationRecipe) recipe).getValidatedResult(
                    new DummyTinkersStationInventory(ts.createStack()),
                    access
            );

            if (rs.hasError()) {

                result = convertLazyResultToToolResult(rs);
                break;
            } else {
    ...
    private RecipeResult<ToolStack> convertLazyResultToToolResult(RecipeResult<LazyToolStack> lazyResult) {
        if (!lazyResult.isSuccess()) {

            return RecipeResult.failure("Modifier validation failed");
        }
    ...
    public CompoundTag toNBT(){
        ...
        nbt.putString("tool", Objects.requireNonNull(tool.getItem().builtInRegistryHolder().key().location()).toString());
    ...
    public static Blueprint fromNBT(CompoundTag tag){
        ResourceLocation toolRL = ResourceLocation.bySeparator(tag.getString("tool"), ':');
        Optional<TCTool> optional = TCTool.getTools().stream()
                .filter(tool -> Objects.equals(tool.getItem().builtInRegistryHolder().key().location(), toolRL)).findFirst();
