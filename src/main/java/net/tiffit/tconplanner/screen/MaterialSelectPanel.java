...
import net.tiffit.tconplanner.util.MaterialRegistryUtils;
...
public class MaterialSelectPanel extends PlannerPanel{
    ...
    public MaterialSelectPanel(int x, int y, int width, int height, PlannerScreen parent) {
        ...
        Class<? extends IMaterialStats> statClass = MaterialRegistryUtils.getClassForStat(part.getStatType());
        if(statClass != null){
            ...
            for (int i = 0; i < sorts.size(); i++) {
                ...
                addChild(new IconButton(startX + i*12, height - 30 + 3, sort.icon, TranslationUtil.createComponent("sort", sort.text), parent, e -> parent.sort(sort))
                        .withColor(sort == parent.sorter ? Color.WHITE : new Color(0.4f, 0.4f, 0.4f))
                        .withSound(SoundEvents.UI_BUTTON_CLICK)
                );}
        }
    }
}