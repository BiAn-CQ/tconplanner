...
import net.tiffit.tconplanner.screen.buttons.modifiers.ModifierLevelInfo;
...
public class ModifierStack {
    ...
    public void setIncrementalDiff(Modifier modifier, int amount) {
        int currentLevel = getLevel(modifier);

        incrementalDiffMap.put(modifier.getId(), Mth.clamp(amount, 0, ModifierLevelInfo.getNeededPerLevelFromEntry(
                                modifier.getId(),
                                currentLevel,
                                amount
                        )
    ...
    public void applyIncrementals(ToolStack tool) {
        stack.stream().distinct().forEach(info -> {
            ...
            int amount = ModifierLevelInfo.getNeededPerLevelFromEntry(
                    mod.getId(),
                    getLevel(mod),
                    getIncrementalDiff(mod)
            );
            if (amount > 0) {
                int finalAmount = amount - getIncrementalDiff(mod);

                CompoundTag persistentData = tool.getPersistentData().getCopy();
                ModifierId modId = mod.getId();
                persistentData.putInt(modId.toString(), finalAmount);
            }
        });
    }
    ...
    public void fromNBT(CompoundTag tag){
        ...
        for(int i = 0; i < modList.size(); i++){
            ...
            if(recipesMap.containsKey(resourceLocation)) {
                ...
                int level = 1;
                push(new ModifierInfo(recipesMap.get(resourceLocation), level));
            }
        }
